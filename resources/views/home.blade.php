...
@section('content')
    ...
    <section class="bg-blue-100 py-16">
        <div class="container mx-auto text-center px-4">
            <h2 class="text-4xl font-bold text-blue-600">Selamat Datang di Toko Kue Kami!</h2>
            <p class="mt-4 text-lg text-gray-700">Rasakan kue terbaik dengan cita rasa yang tak terlupakan.</p>
            <button onclick="scrollToProducts()" class="mt-6 bg-blue-500 text-white py-2 px-4 rounded hover:bg-gray-600 transition">
                Lihat Produk
            </button>
        </div>
    </section>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
                    @foreach ($products as $product)
                        <div class="bg-gray-100 p-6 rounded-lg shadow-md">
                            ...
                            <!-- Harga Produk -->
                            <p class="text-gray-500 font-bold text-lg">Rp {{ number_format($product->price ?? 0, 0, ',', '.') }}</p>
                            <br>
                            <!-- Tombol -->
                            <a href="{{ route('products.show', $product) }}" class="mt-4 bg-blue-500 text-white py-2 px-4 rounded hover:bg-gray-600 transition">
                                Tambah ke Keranjang
                            </a>
                        </div>
                    @endforeach
